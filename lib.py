...
def Lib (con,cur):
    ...
    def Borrowing():
            ...
            if cur.fetchone() is None:
                ...
                if cur.fetchone() is None:
                    ...
                    if role=="student":
                        ...
                        messagebox.showinfo("Error","you are not authorized to borrow")
                        flag=False
                    if flag:
                        if ncopies==0 :
                            Error_399()
                            ...
                            try:
                                ...
                                cur.execute("update book set ncopies = ncopies - 1 where bid = ?",(bid,))
                                con.commit()
        ...
        Button(borrowing, text="Back", width=15, command=backfromb).grid(row=10, column=7, sticky="w", pady=10)
        Button(borrowing, text="Search for user", width=15, command=search_for_user).grid(row=9, column=10, sticky="w", pady=10)
    ...
    def back():
        ...
        textentry.grid(row=1, column=9, sticky="w")
        ...
        textentry.grid(row=4, column=9, sticky="w")

        Button(back, text="Submit", width=15, command=backin).grid(row=9, column=7, sticky="w", pady=10)
        Button(back, text="Back", width=15, command=backfromr).grid(row=10, column=8, sticky="w", pady=10)
        Button(back, text="search for user", width=15, command=search_for_user).grid(row=9, column=9, sticky="w", pady=10)
        #messagebox.showinfo('Note', 'please enter date at this form : YYYY-MM-DD')
        back.mainloop()
    ...
    def borrow_table():
        ...
        for i in range(len(cols)):
            ...
            content.column(cols[i],stretch=YES)
        content.pack(side=TOP)
        cur.execute("select id, book.name as bookname,users.username as username ,bdate,duration from borrow join book on borrow.bid=book.bid join users on borrow.uid=users.uid")
        for i in cur:
